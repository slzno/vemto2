...
function generateRenderablesFile(renderableClasses) {
    const uniqueRenderables = [...new Set(renderableClasses)]
        .filter(Boolean)
        .sort()

    const imports = uniqueRenderables
        .map((renderableClass) => {
            const className = renderableClass.split("/").pop();
            return `import ${className} from './services/${renderableClass}';`
        })
        .join("\n")

    const renderableMap = `
  const renderables = {
    ${uniqueRenderables.map((rc) => {
        const className = rc.split("/").pop();
        return `'${rc}': ${className}`
    }).join(",\n    ")}
  };
  `
...
`
    ...
    console.log(
        `Generated ${outputFile} with ${uniqueRenderables.length} renderables`
    )
