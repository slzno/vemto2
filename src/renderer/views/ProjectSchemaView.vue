<script setup lang="ts">
            ...
            if(table.hasRelatedTables()) {
                ...
                relatedTables.forEach((relatedTable: any) => {
                    let relatedNode = document.getElementById('table_' + relatedTable.name),
                        ...
                        connectionNameReverse = relatedTable.name + '_' + table.name
    ...
    const zoomIn = () => {
        zoom.value += 0.1
        document.getElementById('tablesContainer')!.style.transform = 'scale(' + zoom.value + ')'
    ...
    >
        ...
        <div class="absolute flex top-0 left-0 p-4 space-x-2 text-sm z-20">
            <div class="flex items-center bg-white dark:bg-slate-850 rounded-full shadow px-1">
                <!-- Tools and Icons -->
                <div class="flex">
                    <div class="p-2 cursor-pointer text-slate-400 hover:text-red-500">
            ...
            <div class="flex items-center bg-white dark:bg-slate-850 rounded-full shadow">
                <div class="py-1 px-5 cursor-pointer text-slate-400 hover:text-red-500 flex items-center justify-center">
                    New Schema 
        ...
        <div class="absolute bottom-0 left-0 py-1 px-1 w-full z-20">
            <div class="flex items-center justify-center">
                <div class="flex italic text-sm text-slate-500 hover:text-slate-900 cursor-help">
                    Showing {{ tablesData.length }} of {{ tablesData.length }} tables
                </div>
            </div>
        </div>

        <div class="absolute bottom-0 right-0 p-4 z-20">
            <div class="flex items-center bg-white dark:bg-slate-850 rounded-full shadow p-1">
                <!-- Tools and Icons -->
                <div class="flex">
                    <div class="p-2 cursor-pointer text-slate-600 dark:text-slate-400 hover:text-red-500" @click="zoomIn">
                    ...
                    <div class="p-2 cursor-pointer text-slate-600 dark:text-slate-400 hover:text-red-500" @click="zoomOut">
        ...
        >
            <div
                ...
                :key="table.name"
            >
                <span class="title w-full font-bold text-lg dark:text-slate-300">{{ table.name }}</span>
                ...
                <div class="mt-4 font-mono">
                    ...
                    >
                        <span
                            :class="{
                                'text-yellow-500 dark:text-yellow-400 font-semibold': column.autoIncrement,
                                'text-red-500 dark:text-red-400 font-semibold': column.isForeign(),
                                'text-orange-500 dark:text-orange-400 font-semibold': column.isUnique(),
                            }"
